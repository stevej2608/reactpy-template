...
def Graph(figure: Dict[str, Any], config: Dict[str, Any]):
    traces: List[Any] = []
    ...
    for chart in figure["data"]:
        if "type" in chart:
            ...
            if chart["type"] == "scatter":
                ...
                traces.append(ct)
                continue
            raise ArgumentError(f"Unsupported chart type: {chart['type']}")
        else:
    ...
    fig: Any = go.Figure(data=traces)

    fig.update_layout(
        legend=dict(bgcolor="White", yanchor="top", y=1.00, xanchor="left", x=0.00),
